...
func (h *Handler) bindDefaultEndpoints() {
	...
	// PATCH: enable OIDC for OSS. OIDC (Google SSO) connector handlers. These
	// are normally registered by the enterprise plugin via RegisterProxyWebHandlers.
...
// PATCH: enable OIDC for OSS. The following three handlers implement the OIDC
// (Google SSO) login flow for the proxy web API. They mirror the GitHub
// connector handlers above and are normally provided by the enterprise plugin.
